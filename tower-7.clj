...
(defn parse-line
  [line]
  (let [[details children] (string/split line #" -> ")
        [name* weight] (string/split details #" ")]
    {:name name*
     :weight (Integer/parseInt (subs weight 1 (- (.length weight) 1))) ;; Strip parens
     :children (when children (string/split children #", "))}))

(defn parse-input
  [input-lines]
  (->> (string/split input-lines #"\n")
       (map parse-line)
       (map #(vector (:name %) %))
       (into {})))
...
(def input-map1 (parse-input input-1))

(def input "llyhqfe (21)
...
(def input-map (parse-input input))

(defn parent
  [prog-map prog-name]
  (->> (for [prog (vals prog-map)]
         (when (not= (:name prog) prog-name)
           (if (contains? (set (:children prog)) prog-name)
             (:name prog)
             nil)))
       (remove nil?)
       first))
...
(defn root
  [prog-map]
  (->> (keys prog-map)
       (filter #(nil? (parent prog-map %)))
       first))

(def check-root (partial check root))

(when false
  (check-root "tknk" input-map1)
  (check-root "?" input-map))
...
(defn weight
  [prog-map prog-name]
  (let [prog (get prog-map prog-name)]
    (if (empty? (:children prog))
      (:weight prog))
      (+ (:weight prog)
         (reduce + (map (partial weight prog-map) (:children prog))))))

(when false
  (def check-weight (partial check (partial weight input-map1)))
  (check-weight 251 "ugml")
  (check-weight 243 "padx")
  (check-weight 243 "fwft")
  (check-weight 61 "jptl"))

;; This doesn't work if there isn't actually a bad program
(defn correct-weight
  [prog-map prog-name]
  (loop [off 0
         p prog-name]
    (let [cs (:children (get prog-map p))
          weights (into {} (for [c cs] [c (weight prog-map c)]))]
      (if (apply = (vals weights))
        (+ (:weight (get prog-map p)) off)
        (let [weight-counts (reduce #(update %1 %2 (fnil inc 0)) {} (vals weights))
              bad-weight (some (fn [[k v]] (if (= v 1) k nil)) weight-counts)
              good-weight (some (fn [[k v]] (if (not= v 1) k nil)) weight-counts)
              diff (- good-weight bad-weight)
              bad-child (some (fn [[k v]] (if (= v bad-weight) k nil)) weights)]
          (recur diff bad-child))))))

(def check-correct-weight (partial check (fn [m] (correct-weight m (root m)))))

(when true
  (check-correct-weight 60 input-map1)
  (check-correct-weight "?" input-map))
